...
@section('container')
          ...
          <div class="content-wrapper">
            ...
            <div style="display: flex;align-items: baseline;flex-direction: row;justify-content: space-between;">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{ __('Admin') }} /</span> {{ __('Type') }} </h4>
              @can('type-create')
              <a class="btn btn-success" href="{{ route('admin.type.create',$category_id) }}"> {{ __('Create') }}  </a>
              @endcan
          </div>
              <!-- Basic Bootstrap Table -->
              <div class="card">
                <h5 class="card-header">{{ __('Type') }} </h5>
...
<script src="{{ asset('app/assets/js/vendor/dataTables.select.min.js') }}"></script>
    ...
    $(function () {
        ...
        recordsFiltered : 10,
        ordering: true,
              ajax: "{{ route('admin.type.index',$category_id) }}",
              columns: [
                {data: 'id', name: 'id'},
                  {data: 'name', name: 'name'},
